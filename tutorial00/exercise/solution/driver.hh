...
void driver (const GV& gv, Dune::ParameterTree& ptree)
{
  ...
  auto flambda = [](const auto& x){
    ...
    //== Exercise 2
    // Reverse engineered right hand side for solution verification
    // return -(6*(s-x[0])+9*x[0])*x[0] + s*t;
    return -6*s;
  };
  auto f = Dune::PDELab::
  ...
  //== Exercise 2
  // auto klambda = [](const auto& x){
  //   return x[0];
  // };
  // auto k = Dune::PDELab::makeGridFunctionFromCallable(gv,klambda);
  ...
  //   return s;
  // };
  // auto a = Dune::PDELab::makeGridFunctionFromCallable(gv,alambda);
  ...
  //== Exercise 2
  // typedef PoissonP1<decltype(f),decltype(k),decltype(a),FEM> LOP;
