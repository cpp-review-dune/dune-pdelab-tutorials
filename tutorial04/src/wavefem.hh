    ...
    for (const auto& ip : rule)
      {
        ...
        // evaluate u0
        RF u0=0.0;
        for (size_t i=0; i<lfsu0.size(); i++) u0 += x(lfsu0,i)*phihat[i];
        ...
        for (size_t i=0; i<lfsu0.size(); i++) {
          r.accumulate(lfsu0,i,c*c*(gradu1*gradphi[i][0])*factor); // first equation
          r.accumulate(lfsu1,i,-u0*phihat[i]*factor); // second equation
        }
      }
  }
    ...
    for (const auto& ip : rule)
      {
        ...
        for (size_t j=0; j<lfsu0.size(); j++)
          for (size_t i=0; i<lfsu0.size(); i++) {
            mat.accumulate(lfsu0,i,lfsu1,j,c*c*(gradphi[j][0]*gradphi[i][0])*factor); // first eq.
            mat.accumulate(lfsu1,i,lfsu0,j,-phihat[j]*phihat[i]*factor); // second eq.
          }
      }
  }
...
/** a local operator for the mass operator (L_2 integral) for k identical components
 *
 * \f{align*}{
 \int_\Omega uv dx
 * \f}
    ...
    for (const auto& ip : rule)
      {
        ...
        for (size_t comp=0; comp<LFSU::CHILDREN; comp++)
          {
            // evaluate u
            RF u=0.0;
            for (size_t i=0; i<lfsu0.size(); i++)
              u += x(lfsu.child(comp),i)*phihat[i];

            // integrate u*phi_i
            for (size_t i=0; i<lfsu0.size(); i++)
              r.accumulate(lfsu.child(comp),i,u*phihat[i]*factor);
          }
      }
  }
    ...
    for (const auto& ip : rule)
      {
        ...
        for (size_t j=0; j<lfsu0.size(); j++)
          for (size_t i=0; i<lfsu0.size(); i++)
            for (size_t comp=0; comp<LFSU::CHILDREN; comp++)
              mat.accumulate(lfsu.child(comp),i,lfsu.child(comp),j,phihat[j]*phihat[i]*factor);
      }
  }
