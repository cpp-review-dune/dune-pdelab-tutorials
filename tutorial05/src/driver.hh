...
void driver (Grid& grid, Dune::ParameterTree& ptree)
{
  ...
  // adaptation loop
  // only grid function space and coefficient vector live outside this loop
  ...
  for (int i=0; i<steps; i++)
  {
    ...
    typedef Dune::PDELab::GridOperator<GFS,P0GFS,ESTLOP,MBE,RF,RF,RF,NCC,NCC> ESTGO;
    ...
    std::cout << "Estimated error in step " << i << " is " << estimated_error << std::endl;
    ...
    vtkwriter.write(ptree.get("output.filename",(std::string)"output")+iter,
    ...
    Dune::PDELab::error_fraction(z0,fraction,0.0,eta_refine,eta_coarsen);
