...
\begin{frame} \frametitle{DUNE Release 2.4.1}
...
\mode<presentation>{
  ...
  \mode<presentation>{
  \vspace*{4mm}
  \hspace*{0.5\linewidth}\clap{\fbox{\includegraphics[width=\linewidth]{intro/dunebg}}}
...
%$\frac{1}{|\Omega|}\int_{\Omega}f(x) \diffd x$\\~\hfill$\approx\frac{1}{\sum_{E \in GV} |e|}\sum_{E \in GV}\sum_{i \in QR} f(T_e(x_i)) w_i |\det J_E^T(x_i)|^{1/2}$
\begin{equation*}
...
for (const auto& cell : elements(gv)) {
  ...
  for (auto& ip : Dune::PDELab::quadratureRule(geo,2)) {
    ...
    value += f(x_global) *
             ip.weight() * geo.integrationElement(x_local);
  }
  volume += geo.volume();
}
...
\em
\footnotesize
Note: \lstinline!Dune::PDELab::quadratureRule! is defined in
...
%   Dereferencing \lstinline!IntersectionIterator! yields \lstinline!Intersection!:
%   \begin{columns}
...
\scriptsize
\begin{codeblock}
  \lstinputlisting[lastline=20, basicstyle=\scriptsize\ttfamily]{snippets/indizes_cxx11.cc}
\end{codeblock}
\end{frame}
...
\scriptsize
\begin{codeblock}
  \lstinputlisting[firstline=17, basicstyle=\scriptsize\ttfamily]{snippets/indizes_cxx11.cc}
\end{codeblock}
\end{frame}
\end{onlyenv}
