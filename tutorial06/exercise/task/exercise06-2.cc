...
int main(int argc, char** argv)
{
  try{
    ...
    ptreeparser.readINITree("datahandle.ini",ptree);
    ptreeparser.readOptions(argc,argv,ptree);

    // read ini file
    const int overlap = ptree.get<int>("grid.overlap");
    ...
    L[0] = ptree.get("grid.LX",(double)1.0);
    L[1] = ptree.get("grid.LY",(double)1.0);
    std::array<int,dim> N;
    N[0] = ptree.get("grid.NX",(int)16);
    N[1] = ptree.get("grid.NY",(int)16);
