...
\begin{itemize}
\item The speed of individual computer cores is not increasing essentially since nearly 20 years due to
\begin{itemize}
\item Power wall
\item Memory wall
...
\item Several multi-core processors can be used on one mainboard (e.g. two 32-core processors)
...
The worlds four fastest computers (November 2020) have
\begin{itemize} 
\item Fugaku, Kōbe, Japan: 442.0 PFlop/s\\
7'630'848 cores:
\begin{itemize}
\item 158'976 A64FX48-core processors\\
\end{itemize}
\item Summit, Oak Ridge National Laboratory, U.S.A.: 148.6 PFlop/s\\
202'752 CPU cores + 2'211'840 GPU cores. 4'608 nodes with 
\begin{itemize}
\item 2 IBM Power9 22-core processors each
\item 6 Tesla-Volta 100 80-core accelerator cards each
\end{itemize}
\item Sierra, Lawrence Livermore National Laboratory, U.S.A.: 94.6 PFlop/s\\
190'080 CPU cores + 1'382'400 GPU cores. 4'320 nodes with 
\begin{itemize}
\item 2 IBM Power9 22-core processors
\item 4 Tesla-Volta 100 80-core accelerator cards
\end{itemize}
\item Sunway TaihuLight, Wuxi, China: 93.0 PFlop/s\\
10'649'600 cores:
\begin{itemize}
\item 40'960 SW26010 260-core processors\\
\end{itemize}
\end{itemize}
...
\item A packet consists of an envelope and
  the data:
\begin{center}
  \includegraphics[width=0.4\textwidth]{paket}
\end{center}
...
\item Free Implementations for Linux Clusters: {\bfseries MPICH}
...
\item parallel I/O, one-sided communication and dynamic process management (MPI-2)
\item hybrid-parallelism (MPI-3)
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=0.8\textwidth]{dnapl-3d-het-iso}\\
...
\begin{frame}
  ...
  \begin{center}
    \includegraphics[width=.5\linewidth]{dd}
  \end{center}
  ...
  \begin{columns}
    ...
    \begin{column}{0.4\linewidth}
      \begin{onlyenv}<presentation>
        \includegraphics[width=0.5\linewidth]{konstr_n_uberl_str}
        \vskip5mm
        \includegraphics[width=0.8\linewidth]{konstr_n_uberl_unstr}
      \end{onlyenv}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.4\linewidth}
      \begin{onlyenv}<presentation>
        \includegraphics[width=0.5\linewidth]{konstr_uberl_str}
        \vskip5mm
        \includegraphics[width=0.8\linewidth]{konstr_uberl_unstr}
      \end{onlyenv}
      \begin{onlyenv}<article>
...
\begin{frame} \frametitle{Recapitulation: The Grid: A Container of Entities}
  ...
  \begin{columns}
    ...
    \begin{column}{0.5\linewidth-5mm}
      \begin{onlyenv}<presentation>
        \includegraphics<1-2>[width=0.8\linewidth]{entities}
        \includegraphics<3->[width=0.8\linewidth]{entities_cd}
      \end{onlyenv}
      \begin{onlyenv}<article>
        \vspace*{2ex}
...
\includegraphics[width=0.8\textwidth]{partitionsingle}
\end{column}
...
\only<presentation>{\lstset{basicstyle=\footnotesize\ttfamily,breaklines=true}}
\lstinputlisting[lastline=10,breaklines=true]{example01_main.hh}
...
using FEM = Dune::PDELab::Q1LocalFiniteElementMap<Coord,Real,dim>;
FEM fem;
!\color{red}using CON = Dune::PDELab::OverlappingConformingDirichletConstraints;!
...
using Param = ConvectionDiffusionProblem<GV,Real>;
Param param;
using B = Dune::PDELab::BCTypeParam_CD<Param>;
B b(gv,param);
using G = Dune::PDELab::DirichletBoundaryCondition_CD<Param>;
G g(gv,param);

//  Compute constrained space
using C = typename GFS::template ConstraintsContainer<Real>::Type;
C cg;
Dune::PDELab::constraints(b,gfs,cg);
// Make grid operator
using LOP = Dune::PDELab::ConvectionDiffusion<Param>;
LOP lop(param,2);
...
!\color{red}using LS = Dune::PDELab::ISTLBackend\_OVLP\_BCGS\_SuperLU<GFS,C>;!
LS ls(gfs,cg,5000,2);
...
\end{lstlisting}
\end{frame}
...
using FEM = Dune::PDELab::Q1LocalFiniteElementMap<Coord,Real,dim>;
FEM fem;
!\color{red}using CON = Dune::PDELab::NonoverlappingConformingDirichletConstraints<GV>;!
...
LOP lop(param,2);
using MBE = Dune::PDELab::ISTLMatrixBackend;
using GO = Dune::PDELab::GridOperator<GFS,GFS,LOP,MBE,double,double,double,C,C,true>;
...
// 3. Choose a linear solver
!\color{red}using LS = Dune::PDELab::ISTLBackend\_NOVLP\_BCGS\_SSORk<GO>;!
LS ls(go,5000,3,2);
...
\end{lstlisting}
